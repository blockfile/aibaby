...
async function createCycle({ dryRun }) {
  ...
  await db.collection('cycles').insertOne({
    ...
    // this flow, so no *_eth columns. tokens_burned is ARTCAT: the only
 ...
 * can show "874,649 ARTCAT destroyed" beside a link to the burn itself. The
 ...
 * wallet and may hold ARTCAT personally, which must never be burned.
 */
