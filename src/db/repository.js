...
async function createCycle({ dryRun }) {
  ...
  await db.collection('cycles').insertOne({
    ...
    // this flow, so no *_eth columns. tokens_burned is BABYINU: the only
...
async function getStats() {
  ...
  // Sum claimed NVDA from the claim STEPS, not the cycles: a step is recorded
 ...
 * can show "874,649 BABYINU destroyed" beside a link to the burn itself. The
 ...
 * wallet and may hold BABYINU personally, which must never be burned.
 */
