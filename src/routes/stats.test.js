...
const build = (market, token, rewards = {}, curve = {}, rewardPrice = {}, supply = null) =>
  buildStats({
    ...
    quote: rewardPrice, symbol: 'CAT', tokenAddress: '0xabc', supply,
  });
...
test('burn figures are null (never 0) when nothing has been burned yet', () => {
  const out = buildStats({ market: {}, token: {}, symbol: 'CAT' });
...
const CAT = '0xc6e8c393d46b685c2fb2177f759f2b16eb7a7d54';
const pair = (quote, price, mcap, liq) => ({
  chainId: 'robinhood',
  baseToken: { address: CAT },
...
test('the liquidity floor still applies within the launch pair', () => {
  ...
  const out = parsePairs({ pairs: [pair(NVDA, 0.5, 500_000_000, 3.32)] }, CAT, 'robinhood');
...
test('fees earned with no price gives a null USD, not a zero', () => {
  const out = buildStats({
    ...
    quote: {}, symbol: 'CAT', tokenAddress: '0xtoken',
  });
...
test('no creator-fee data at all leaves the tile hidden rather than zeroed', () => {
  const out = buildStats({ market: {}, token: {}, rewards: {}, symbol: 'CAT', tokenAddress: '0xtoken' });
