...
function lore({ symbol, rewardSymbol }) {
  return [
    ...
    `$${symbol} is that experiment, shipped. Every half hour the fees the network captures buy $${rewardSymbol}, and the $${rewardSymbol} is routed straight to the holders — no treasurer, no multisig theatre, no dev wallet quietly draining in the background. The cat runs the loop. The loop pays out in the open.`,
  ];
}
